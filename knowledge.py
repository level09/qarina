...
def _get_rag():
    ...
    with _lock:
        ...
        try:
            ...
            _rag = rag
            log.info(f"LightRAG initialized at {KNOWLEDGE_DIR}")
            return _rag
...
def get_prior_knowledge(query: str) -> str | None:
    ...
    try:
        ...
        skip = ("sorry", "not able to", "no-context", "no context", "i don't have", "no information")
...
def shutdown():
    global _rag, _loop, _init_failed
    if _rag:
        try:
            _run(_rag.finalize_storages())
        except Exception:
            pass
        _rag = None
